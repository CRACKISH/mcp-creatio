...
function req(opts: { query?: Record<string, unknown>; body?: unknown; headers?: Record<string, string> }) {
	return {
		query: opts.query ?? {},
		body: opts.body,
...
function res() {
	const r: Record<string, unknown> = { statusCode: 200, headers: {}, redirectedTo: undefined, jsonBody: undefined };
...
describe('BrokerHandlers — full authorization_code broker flow', () => {
	it('register → authorize → callback → token → mcpAuth, end to end', async () => {
		...
		const regRes = res();
		h.handleRegister(req({ body: { redirect_uris: [CLIENT_REDIRECT] } }) as never, regRes as never);
		...
		vi.stubGlobal('fetch', vi.fn(async () => jsonOk({ access_token: creatioToken, refresh_token: 'RT', expires_in: 3600 })));
		const cbRes = res();
		await h.handleCallback(req({ query: { code: 'creatio-code', state: brokerState } }) as never, cbRes as never);
		...
		expect((await SessionContext.instance.getTokensForUser('creatio-user-1'))?.accessToken).toBe(creatioToken);
		...
		const next2 = vi.fn();
		const r2 = res();
		await h.mcpAuth()(req({ headers: { authorization: `Bearer ${ourToken}` } }) as never, r2 as never, next2);
		expect(next2).not.toHaveBeenCalled();
		expect(r2.statusCode).toBe(401);
		expect((r2.headers as Record<string, string>)['WWW-Authenticate']).toContain('error="invalid_token"');
	});
	...
	it('mcpAuth rejects a missing token with a WWW-Authenticate challenge', async () => {
		...
		expect((r.headers as Record<string, string>)['WWW-Authenticate']).toContain('resource_metadata=');
	...
	it('token exchange fails for a wrong PKCE verifier', async () => {
		...
		const authRes = res();
		await h.handleAuthorize(
			req({ query: { client_id: clientId, redirect_uri: CLIENT_REDIRECT, response_type: 'code', code_challenge: challenge, code_challenge_method: 'S256' } }) as never,
			authRes as never,
		);
		const brokerState = new URL(authRes.redirectedTo as string).searchParams.get('state')!;
		vi.stubGlobal('fetch', vi.fn(async () => jsonOk({ access_token: jwt.sign({ sub: 'u' }, 'x'), expires_in: 3600 })));
		const cbRes = res();
		await h.handleCallback(req({ query: { code: 'c', state: brokerState } }) as never, cbRes as never);
		const ourCode = new URL(cbRes.redirectedTo as string).searchParams.get('code')!;
		const tokRes = res();
		await h.handleToken(
			req({ body: { grant_type: 'authorization_code', code: ourCode, redirect_uri: CLIENT_REDIRECT, client_id: clientId, code_verifier: 'WRONG-VERIFIER' } }) as never,
			tokRes as never,
		);
